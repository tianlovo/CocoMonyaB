...
import type { PageResponse, Work } from '@/types/models'

vi.mock('@/api/work')

describe('Work Store - Property-Based Tests', () => {
  beforeEach(() => {
   ...
   * 
   * Property: For any search keyword input, when fetching works through the store,
    ...
    it('should pass keyword parameter to API when fetching page with keyword', async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          async (params) => {
            const mockResponse: PageResponse<Work> = {
              records: [],
            ...
            vi.mocked(workApi.getPage).mockResolvedValue(mockResponse)

            const store = useWorkStore()
            await store.fetchPage(params)

            // Verify the keyword was passed to the API
            expect(workApi.getPage).toHaveBeenCalledWith(
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (params) => {
            ...
            const callArgs = vi.mocked(workApi.getPage).mock.calls[
              vi.mocked(workApi.getPage).mock.calls.length - 1
            ][0]
        ...
        fc.asyncProperty(
          ...
          async (paramsList) => {
            ...
            for (const params of paramsList) {
              ...
              const lastCall = vi.mocked(workApi.getPage).mock.calls[
                vi.mocked(workApi.getPage).mock.calls.length - 1
              ][0]
        ...
        fc.asyncProperty(
          fc.record({
            ...
            keyword: fc.oneof(
              fc.constant('原作名'),
              fc.constant('作品 (别名)'),
              fc.constant('Work@example'),
              fc.constant('测试-原作_123'),
              fc.constant('Work with 中文')
            )
          }),
          async (params) => {
            const mockResponse: PageResponse<Work> = {
