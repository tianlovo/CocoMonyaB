...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (error?.code === -60003) {
      handleUniqueConflictError(error.message)
    } else {
...
// Handle uniqueness conflict error
const handleUniqueConflictError = (message: string) => {
  // Parse error message to extract conflict details
  // Format: "名称已存在：冲突实体类型=AUTHOR, ID=xxx, 名称=xxx"
  const match = message.match(/冲突实体类型=(\w+),\s*ID=([^,]+)/)
  
  if (match) {
    const [, entityType, entityId] = match
    const entityTypeName = getEntityTypeName(entityType)
    ElMessage.error({
      message: `该名称已被其他${entityTypeName}使用（ID: ${entityId}）`,
      duration: 5000
    })
  } else {
    ElMessage.error({
