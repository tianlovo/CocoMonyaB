...
/**
 * 远程模式集成测试
 * 验证应用在远程MongoDB模式下能够正常启动、初始化数据库和执行CRUD操作
 * 
 * 注意：此测试需要实际的远程MongoDB服务器
 * 可以通过设置环境变量 RUN_REMOTE_MONGO_TESTS=true 和 MONGO_TEST_URI 来启用此测试
 * 
 * Requirements: 1.3, 1.5, 3.1, 3.2, 3.3, 3.4
 */
@SpringBootTest
@TestPropertySource(properties = {
        "spring.data.mongodb.mode=remote",
        "spring.data.mongodb.uri=${MONGO_TEST_URI:mongodb://localhost:27017/cocomonya_test}"
})
@EnabledIfEnvironmentVariable(named = "RUN_REMOTE_MONGO_TESTS", matches = "true")
class RemoteModeIntegrationTest {
    ...
    /**
     * 测试应用在远程模式下成功启动
     * Requirement 1.3: 远程模式初始化
     * Requirement 1.5: 使用spring.data.mongodb.uri连接
     */
    @Test
    void testApplicationStartsSuccessfullyWithRemoteMode() {
    ...
    /**
     * 测试CRUD操作在远程MongoDB下正常工作
     * Requirement 1.3: 远程模式功能完整性
     */
    @Test
    void testCrudOperationsWorkWithRemoteMongoDB() {
        // 清理测试数据
        ...
        try {
            ...
            createDTO.setChannelId(2001L);
            createDTO.setChannelUsername("remote_test_channel");
            createDTO.setChannelTitle("Remote Test Channel");
            ...
            assertEquals(2001L, created.getChannelId(), "channelId应该匹配");
            assertEquals("remote_test_channel", created.getChannelUsername(), "channelUsername应该匹配");
            assertEquals("Remote Test Channel", created.getChannelTitle(), "channelTitle应该匹配");
            ...
            updateDTO.setChannelTitle("Updated Remote Test Channel");
            ...
            assertEquals("Updated Remote Test Channel", updated.getChannelTitle(), "channelTitle应该被更新");
            assertFalse(updated.getMonitoringStatus(), "monitoringStatus应该被更新为false");
            assertEquals("remote_test_channel", updated.getChannelUsername(), "未更新的字段应该保持不变");
            ...
            assertTrue(list.size() >= 1, "应该至少有1个channel");
            
            // When: 删除channel
            channelService.deleteById(created.getId());
            
            // Then: 删除成功，该channel不应再存在
            List<ChannelVO> afterDelete = channelService.list();
            boolean channelExists = afterDelete.stream()
                    .anyMatch(ch -> ch.getId().equals(created.getId()));
            assertFalse(channelExists, "删除后该channel不应存在");
        } finally {
            // 清理测试数据
            channelRepository.deleteAll();
        }
